...
*/
#define TGL_OPTIMIZATION_HINT_BRANCH_COST 0
...
/*
 * Byte-swap configuration for displays expecting big-endian RGB565 on the wire
 * (e.g. ST7735 with MADCTL_RGB and no controller-side swap).
 * When 1, RGB_TO_PIXEL / gl_convertRGB_to_5R6G5B emit byte-swapped 16-bit
 * words so a little-endian CPU (ESP32) lays them out big-endian in RAM.
 */
#ifndef TGL_PIXEL_BYTE_SWAP
#define TGL_PIXEL_BYTE_SWAP 0
#endif
#ifndef TGL_TEXTURE_BYTE_SWAP
#define TGL_TEXTURE_BYTE_SWAP TGL_PIXEL_BYTE_SWAP
#endif
#if TGL_PIXEL_BYTE_SWAP == 1
#include <stdint.h>
#define TGL_BSWAP16(x) ((uint16_t)(((uint16_t)(x) << 8) | (((uint16_t)(x)) >> 8)))
#else
#define TGL_BSWAP16(x) (x)
#endif
