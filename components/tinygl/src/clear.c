...
void glopClear(GLParam* p) {
	GLContext* c = gl_get_context();
	GLint mask = p[1].i;
	/* Use z=0 for Z clear (near plane). TinyGL uses z >= zpix test,
	 * so clearing to 0 ensures all geometry passes the depth test.
	 * (Equivalent to GL_GEQUAL depth func with clear-to-near-plane.) */
	GLint z = 0;
